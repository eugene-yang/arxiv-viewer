#!/usr/bin/env python3
"""
ArXiv Digest Viewer - Two Panel Layout

A Streamlit web interface for browsing arXiv digest files with:
- Compact title list for quick scanning
- Detail panel on click with abstract and auto-generated TLDR
- Progress tracking per digest

Usage:
    streamlit run arxiv_viewer.py -- --digest-dir ./digests
    streamlit run arxiv_viewer.py -- --digest-dir ./digests --ollama-model llama3.2
"""
...
PAPERS_PER_PAGE = 10  # More papers per page since we're showing compact list
...
def load_progress(log_file: Path) -> dict:
    """Load reading progress from log file."""
    if log_file.exists():
        try:
...
def save_progress(log_file: Path, progress: dict):
    """Save reading progress to log file."""
...
def generate_tldr(
    ...
    """Generate a one-liner TLDR using Ollama."""
...
def list_digest_files(digest_dir: Path) -> list[dict]:
    """List all digest files with metadata."""
    digests = []
...
def load_digest(filepath: Path) -> tuple[dict, list[dict]]:
    """Load a digest file, returning (metadata, papers)."""
    metadata = {}
    papers = []
...
def main():
    ...
    # Page config - wide layout for two panels
    st.set_page_config(
    ...
    st.markdown("""
    <style>
    /* Add top padding to avoid Deploy banner */
    .block-container {
        padding-top: 3rem;
        padding-bottom: 1rem;
    }

    /* Paper list buttons - full title, left aligned, wrap text */
    ...
    # Sidebar: digest selection and settings
    with st.sidebar:
    ...
    with col_list:
        ...
        for i, paper in enumerate(page_papers):
            ...
            # Create clickable button for each paper
            is_selected = st.session_state.selected_paper_idx == global_idx

            # Format: #rank [score] title
    ...
    with col_detail:
        if st.session_state.selected_paper_idx is not None:
            ...
            # No paper selected - show instructions and stats
            st.markdown("### 👈 Select a paper")
            st.markdown("Click on a paper title from the list to view details and auto-generate TLDR.")
    ...
    papers_on_current_page = min(
        (st.session_state.current_page + 1) * PAPERS_PER_PAGE,
        len(papers)
    )
    current_progress["papers_seen"] = max(
        current_progress["papers_seen"],
        papers_on_current_page
    )
